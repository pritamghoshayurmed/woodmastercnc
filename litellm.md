...
**LiteLLM** is an open-source library that gives you a single, unified interface to call 100+ LLMs â OpenAI, Anthropic, Vertex AI, Bedrock, and more â using the OpenAI format.

- Call any provider using the same `completion()` interface â no re-learning the API for each one
...
**Want to get started fast?** Head to [Tutorials](/docs/tutorials) for step-by-step walkthroughs â AI coding tools, agent SDKs, proxy setup, and more.
...
LiteLLM maps every provider's errors to the OpenAI exception types â your existing error handling works out of the box:
...
ð [See all observability integrations â](/docs/observability/agentops_integration)
...
The proxy is a self-hosted OpenAI-compatible gateway. Any client that works with OpenAI works with the proxy â no code changes needed.

![LiteLLM Proxy Dashboard](https://github.com/BerriAI/litellm/assets/29436595/47c97d5e-b9be-4839-b28c-43d7f4f10033)

#### Step 1 â Start the proxy
...
#### Step 2 â Call it with the OpenAI client
...
ð [Full proxy quickstart with Docker â](./proxy/docker_quick_start)

:::tip[Debugging tool]
Use [**`/utils/transform_request`**](./utils/transform_request) to inspect exactly what LiteLLM sends to any provider â useful for debugging prompt formatting, header issues, and provider-specific parameters.
:::
...
LiteLLM is a unified gateway for **LLMs, agents, and MCP** â you don't need a separate agent or MCP gateway. One endpoint for 100+ models, A2A agents, and MCP tools.
...
description: "Quickstart guide for trial customers â LLM, MCP, and Agent gateway.",
...
client = OpenAI(
  ...
  api_key = "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"
)
